...
base_folder_path = os.path.join("details", current_year, "seoul")
...
time.sleep(2)
browser.find_element(By.CSS_SELECTOR, ".loca_step1_cont .sido_arae_box li:nth-child(1)").click()  # 서울
time.sleep(2)
...
for index, store in enumerate(stores):
    try:
        ...
        # ✅ 영업시간 로드 대기
        try:
            wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, ".date_time dl dd")))
            time.sleep(0.8)
        except:
            pass

        # BeautifulSoup으로 기본 정보 파싱
        ...
        try:
            ...
            store_hours = list(dict.fromkeys(store_hours))  # 중복 제거
        ...
        # 💾 100개 단위 자동 저장
        if (index + 1) % 100 == 0:
            part_num = (index + 1) // 100
            part_path = os.path.join(base_folder_path, f"seoul_{current_date}_part{part_num}.json")
            ...
            print(f"💾 {index + 1}개 저장 완료 → {part_path}")
            store_data_list = []  # 메모리 초기화
...
# ==============================
if store_data_list:
    last_path = os.path.join(base_folder_path, f"seoul_{current_date}_last.json")
    ...
    print(f"💾 마지막 {len(store_data_list)}개 저장 → {last_path}")
